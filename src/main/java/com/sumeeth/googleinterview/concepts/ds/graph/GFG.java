package com.sumeeth.googleinterview.concepts.ds.graph;// Java program to print the largest
...
// connection with key = input[i][j]
    static void BFS(int keyX, int keyY, int i,
                    int j, int input[][]) {
		...
		// top/bottom move can be done using x_move as
		// left/right move can be done using y_move
        for (int u = 0; u < 4; u++)
            if (is_valid(i + y_move[u],
                    j + x_move[u], keyX, input))
                BFS(keyX, keyY, i + y_move[u],
                        j + x_move[u], input);
    }
...
// largest connected component
    ...
    public static void main(String args[]) {
        ...
        computeLargestConnectedGrid(input2);
    }
}
